...
async function createPaymentLink(productId, customerPhoneNumber) {
    try {
        ...
        const userRedirectURL = `https://call-a-bot.onrender.com/chat?sessionId=${session.sessionId}&token=${userToken}&room=${roomID}`;
        const vetRedirectURL = `https://call-a-bot.onrender.com/chat?sessionId=${session.sessionId}&token=${vetToken}&room=${roomID}`;
